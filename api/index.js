...
function sha256Sync(ascii) {
  ...
  for (i = 0; i < ascii[lengthProperty]; i++) {
    ...
    words[i >> 2] |= (charCode & 255) << 24 - i % 4 * 8;
  }
...
var PASSWORD_SALT_PREFIX = "$sapa$v1$";
var STATIC_SECRET_PEPPER = "sapa_salt_counseling_2026_smk_sec_";
...
function verifyPassword(inputPassword, storedPasswordOrHash) {
  if (!storedPasswordOrHash) return true;
...
var NIP_ENCRYPT_PREFIX = "$sapa$nip$v1$";
var NIP_SECRET_PEPPER = "sapa_nip_cipher_key_2026_smk_bk_sec_";
function decryptNip(encryptedOrPlainNip) {
...
function inspectConnectionString(connStr) {
  if (!connStr) {
    return {
      hasUrl: false,
  ...
  try {
    ...
    if (parsed.password && (parsed.password.includes("[") || parsed.password.includes("]") || parsed.password.toUpperCase().includes("YOUR-PASSWORD") || parsed.password.toUpperCase().includes("YOUR_PASSWORD"))) {
...
function getOptimizedPrismaUrl(rawUrl) {
  if (!rawUrl) return void 0;
  try {
    const url = new URL(rawUrl);
    const isPooler = url.port === "6543" || url.hostname.includes("pooler") || url.hostname.includes("neon") || url.searchParams.has("pgbouncer");
...
var lastPrismaError = null;
var lastPrismaPingMs = null;
...
async function resetPrismaClient() {
  ...
  globalThis.__prismaClientInstance = void 0;
}
...
async function withPrismaRetry(fn, maxRetries = 2) {
  let lastError = null;
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (msg.includes("Closed") || msg.includes("kind: Closed") || msg.includes("connection was closed") || msg.includes("Can't reach database server") || msg.includes("P1001") || msg.includes("terminating connection")) {
...
async function testPrismaDatabase() {
  ...
  try {
    ...
    const pingPromise = (async () => {
      ...
      await prisma2.$queryRawUnsafe("SELECT 1 as ping");
      return prisma2;
    })();
    ...
    try {
      ...
      const countTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error("COUNT_TIMEOUT")), 2e3));
    ...
    const isTimeout = err.message.includes("TIMEOUT_4500MS") || err.message.includes("timeout") || err.message.includes("P1001") || err.message.includes("Can't reach database server");
    if (isTimeout) {
    ...
    return {
      ...
      isPooler: info.isPooler,
      hasDatabaseUrl: true,
      error: err.message.includes("TIMEOUT_4500MS") ? "Batas waktu koneksi habis (4.5s). Database tidak dapat dijangkau dari server Vercel." : err.message,
      timestamp: now,
      warnings,
      recommendations
    };
...
function getMaskedDbInfo(connStr) {
  if (!connStr) {
    return {
      hasUrl: false,
      host: "none",
...
async function ensureDbInitialized(forceRetry = false) {
...
async function testDatabaseConnection() {
  try {
    ...
    if (prismaResult.error && (prismaResult.error.includes("Query engine") || prismaResult.error.includes("binaryTarget") || prismaResult.error.includes("PrismaClient") || prismaResult.error.includes("cannot be found"))) {
...
async function updateUserPassword(userId, newHashedPassword, passwordChanged = true) {
...
async function getUserByIdentifier(identifier) {
  ...
  if (isPostgresConnected && pool) {
    ...
    const s = memoryStore.students.find((s2) => s2.nis.toLowerCase() === trimmed.toLowerCase());
...
async function getTeachers() {
  if (isPostgresConnected && pool) {
    ...
    return res.rows.map((r) => ({
      ...r,
      assigned_class_ids: Array.isArray(r.assigned_class_ids) ? r.assigned_class_ids : typeof r.assigned_class_ids === "string" ? JSON.parse(r.assigned_class_ids) : []
    }));
  }
...
function enrichReport(report, categories, students, users, classes, messages, teachers) {
  ...
  let specificName = void 0;
  let specificAvatar = void 0;
  let specificRoom = void 0;
  ...
  if (filter?.role === "siswa" && filter.userId) {
    ...
    if (filter.teacherType === "wali_kelas" && filter.classId) {
      ...
      reports = reports.filter(
        (r) => r.assigned_to === "wali_kelas" && classStudents.includes(r.student_id) || r.urgency === "tinggi" && classStudents.includes(r.student_id)
      );
  ...
  if (isPostgresConnected && pool) {
    await pool.query(
      ...
      [
        ...
        newReport.category_id,
        newReport.assigned_to,
        newReport.assigned_teacher_id,
        newReport.title,
...
async function updateReportStatus(reportId, newStatus, changedByUserId, note) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function addMessage(reportId, senderId, messageText) {
  ...
  if (senderRole === "siswa" && reportStatus && (reportStatus === "direspons" || reportStatus === "ditindaklanjuti" || reportStatus === "selesai")) {
...
async function getBkTeachers() {
  ...
  return teachers.map((t, idx) => {
    ...
    const statuses = ["tersedia", "tersedia", "tersedia"];
    return {
      teacher_id: t.id,
      user_id: t.user_id,
...
apiRouter.post("/admin/seed-supabase", async (req, res) => {
  try {
    const pool2 = getPool();
    if (!pool2) {
...
apiRouter.all("/admin/test-db", async (req, res) => {
  try {
...
apiRouter.get("/admin/db-diagnostics", async (req, res) => {
  try {
    const connStr = getConnectionString();
...
apiRouter.post("/users/:id/reset-password", async (req, res) => {
  try {
    ...
    const finalHash = hashedPassword || (password ? hashPassword(password) : void 0);
    if (!finalHash) {
      return res.status(400).json({ error: "Kata sandi baru wajib disediakan" });
    }
    const changed = password_changed !== void 0 ? password_changed : user.role === "siswa" ? false : true;
...
apiRouter.post("/users/:id/change-password", async (req, res) => {
  try {
    const { id } = req.params;
...
apiRouter.get("/teachers/bk", async (req, res) => {
  try {
